...
Mesh* MeshBuilder::GenerateBackground(const std::string &meshName, Color color, float repeat)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
...
Mesh* MeshBuilder::GenerateTileSheet(const std::string &meshName, unsigned numRow, unsigned numCol)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
...
Mesh* MeshBuilder::GenerateSkyPlane(const std::string &meshName, Color color, int slices, float PlanetRadius, float AtmosphereRadius, float hTile, float vTile)
{
	...
	//Check wheter no. of slices is within 1 - 256
...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, std::vector<unsigned char>& heightMap)
{
	...
	for (unsigned int z = 0; z < terrainSize; ++z)
	{
		for (unsigned int x = 0; x < terrainSize; ++x)
		{
			...
			v.pos.Set((float)x / terrainSize - 0.5f, scaledHeight, (float)z / terrainSize - 0.5f);
	...
	for (unsigned int i = 0; i < terrainSize - 1; ++i)
	{
		for (unsigned int j = 0; j < terrainSize - 1; ++j)
		{
			...
			p1 = vertex_buffer_data[i * terrainSize + j + 1].pos;
			p2 = vertex_buffer_data[(i + 1) * terrainSize + j].pos;
			...
			vertex_buffer_data[i * terrainSize + j].normal += norm;
			vertex_buffer_data[i * terrainSize + j + 1].normal += norm;
			vertex_buffer_data[(i + 1) * terrainSize + j].normal += norm;
	...
	for (unsigned int i = 0; i < vertex_buffer_data.size(); ++i)
	{
		if (vertex_buffer_data[i].normal.x != 0 &&
			...
			vertex_buffer_data[i].normal.z != 0)
			vertex_buffer_data[i].normal.Normalize();
	}
...
Mesh* MeshBuilder::GenerateMinimap(const std::string &meshName, Color color, float length, float texSize)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(1.f * texSize, 0);
	vertex_buffer_data.push_back(v);

	v.pos.Set(0.5f * length, 0.5f * length, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(1.f * texSize, 1.f * texSize);
	vertex_buffer_data.push_back(v);

	v.pos.Set(-0.5f * length, 0.5f * length, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, 1.f * texSize);
	vertex_buffer_data.push_back(v);
...
Mesh* MeshBuilder::GenerateCircle(const std::string &meshName, Color color, unsigned numSlices)
{
	...
	for (unsigned slice = 0; slice < numSlices + 1; slice++)
	{
		...
		v.pos.Set(CircleX(theta), CircleY(theta), 0);
		v.texCoord.Set(CircleX(theta), CircleY(theta));
		v.normal.Set(0, 1, 0);
		v.color = color;
		vertex_buffer_data.push_back(v);
	}
