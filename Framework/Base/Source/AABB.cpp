...
/*!
\brief
Sets the AABB's Min and Max by using an object position and using the size as distance away from object.

\param position - Center of object
\param size - Size of AABB
*/
/******************************************************************************/
void AABB::Set(Vector3 position, float size)
{
	Min.Set(position.x - size, position.y - size, position.z - size);
	Max.Set(position.x + size, position.y + size, position.z + size);
}
