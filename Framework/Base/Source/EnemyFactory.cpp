...
Enemy* EnemyFactory::Create(const string& name, Vector3 pos, TileMap* map)
{
	...
	if (name == "BigDemon")
	{
		...
		enemy->SetAlertRange(100.f);
		enemy->SetPatrolRange(300.f);
		enemy->SetDefaultSpeed(130.f);
		enemy->SetAttackDamage(65.f);
		enemy->SetAttackRange(40.f);
		enemy->SetTimeBetweenAttack(0.5f);
		enemy->SetHealth(800);
		enemy->SetElement(FIRE, 20);
		enemy->active = true;
		GoManager::GetInstance().Add(enemy);
		return enemy;
	}
	...
	if (name == "RangeDemon")
	{
		...
		enemy->SetWalkAnim(0, 6, 1.f);
		enemy->SetAlertRange(200.f);
		enemy->SetPatrolRange(150.f);
		enemy->SetAttackRange(200.f);
